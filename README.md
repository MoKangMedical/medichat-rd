...
# 启动Demo服务（含罕见病诊断+社群+知识库）
python3 demo_server.py
# → http://localhost:8000
# → API文档: http://localhost:8000/docs
```

### 方式二：完整后端（需配置MIMO API Key）
...
### 方式三：Docker Compose（全栈，含数据库）

```bash
# 需要先创建 nginx 配置目录（docker-compose 依赖）
mkdir -p nginx && cp /dev/null nginx/nginx.conf  # 或自定义配置
docker-compose up -d
# → http://localhost:80
```
